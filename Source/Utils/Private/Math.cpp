...
glm::mat4 Math::Perspective(const float verticalFov, const float aspectRatio, const float near, const float far,
    const bool reverseDepth /* = true */, const bool flipY /* = true */)
{
    // TODO: Infinite far
    ...
    matrix[1][1] = near / top * (flipY ? -1.0f : 1.0f);
    matrix[2][2] = reverseDepth ? near / (near - far) : -(far + near) / (far - near);
    matrix[2][3] = -1.0f;
    matrix[3][2] = reverseDepth ? -(far * near) / (near - far) : -(2.0f * far * near) / (far - near);
