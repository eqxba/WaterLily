#include "Engine/Render/Vulkan/DescriptorSets/DescriptorSetPool.hpp"
    ...
    : vulkanContext{ &aVulkanContext }
{
    ...
    vkCreateDescriptorPool(vulkanContext->GetDevice(), &poolCreateInfo, nullptr, &pool);
}
...
std::tuple<VkResult, VkDescriptorSet> DescriptorSetPool::Allocate(const VkDescriptorSetLayout layout) const
{
    ...
    const VkResult result = vkAllocateDescriptorSets(vulkanContext->GetDevice(), &allocateInfo, &descriptorSet);
...
void DescriptorSetPool::Reset() const
{
    if (pool != VK_NULL_HANDLE)
    {
        vkResetDescriptorPool(vulkanContext->GetDevice(), pool, 0);
    }
}
